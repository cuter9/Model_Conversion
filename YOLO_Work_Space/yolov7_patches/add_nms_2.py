...
class RegisterNMS(object):
    ...
    def infer(self):
        ...
        """
        for ni in range(5):
            LOGGER.info("No of times of constant folding inferring : %d" % ni)
    ...
    def save(self, output_path):
        ...
        """
        # nd_nms = [nd for nd in self.graph.nodes if nd.name == 'batched_nms'][0]
        # self.graph.outputs = nd_nms.outputs
        self.graph.cleanup().toposort()
        model = gs.export_onnx(self.graph)
    ...
    def register_nms(
        ...
        nd_nms = gs.Node(op=op, name="batched_nms", inputs=op_inputs, outputs=op_outputs, attrs=attrs)
        self.graph.nodes.append(nd_nms)
