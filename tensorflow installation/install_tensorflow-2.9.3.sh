...
if [[ ! $(head -1 /etc/nv_tegra_release) =~ R32.*7\.[345] ]] ; then
...
sudo pip3 install -U "numpy<1.24.0"	# install numpy1.23.5
...
if [ ! -d "./tensorflow-${version}" ]; then
  echo "** downloading tensorflow-${version}"
  wget https://github.com/tensorflow/tensorflow/archive/refs/tags/v${version}.tar.gz -O tensorflow-${version}.tar.gz
